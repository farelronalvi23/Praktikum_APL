...
// FIX: Saat biayaFinal diisi, status selesai otomatis jadi true
void adminUbahServis(Servis daftar[], int jml) {
    ...
    cin.ignore(); // flush newline setelah cin >> nomor
    ...
    cout << "\n  [ Ubah Pemilik ]\n";
    cout << "  Nama  [" << daftar[idx].pemilik.nama        << "]: "; getline(cin,inp); if(!inp.empty()) daftar[idx].pemilik.nama=inp;
    cout << "  Telp  [" << daftar[idx].pemilik.noTelepon   << "]: "; getline(cin,inp); if(!inp.empty()) daftar[idx].pemilik.noTelepon=inp;

    cout << "\n  [ Ubah Servis ]\n";
    cout << "  Jenis       [" << daftar[idx].jenisServis    << "]: "; getline(cin,inp); if(!inp.empty()) daftar[idx].jenisServis=inp;
    cout << "  Est.Biaya   [" << daftar[idx].biayaEstimasi  << "]: "; getline(cin,inp); if(!inp.empty()) daftar[idx].biayaEstimasi=stoi(inp);

    // FIX: Biaya Final diisi -> otomatis set selesai = true
    cout << "  Biaya Final [" << daftar[idx].biayaFinal     << "]: "; getline(cin,inp);
    if (!inp.empty()) {
        daftar[idx].biayaFinal = stoi(inp);
        daftar[idx].selesai = true; // otomatis tandai selesai saat biaya final diisi
        tampilkanPesan("*", "Status otomatis diubah ke [SELESAI] karena biaya final sudah diisi.");
    }

    // Tetap bisa override status manual jika perlu
...
void userLihatStatus(Servis daftar[], int jml, User aktif) {
    ...
    if (!ada) {
        ...
        cout << "  Total biaya final : Rp " << hitungTotalBiaya(0, jml, daftar, aktif.nama) << "\n";
        int kunjungan = no - 1;
        if (kunjungan >= 2) {
...
void userRequestServis(Servis daftar[], int &jml, User aktif) {
    ...
    tampilkanPesan("Admin akan segera memproses ya KANDA KUHHH!!!");
}
...
void menuAwal() {
    ...
    do {
        ...
        switch (p) {
            ...
            case 0: tampilkanPesan("Terima kasih sudah menggunakan program ini abang/mbaa kuhhhh!"); break;
            default: tampilkanPesan("!", "Pilihan tidak valid!");
        }
    } while (p != 0);
}