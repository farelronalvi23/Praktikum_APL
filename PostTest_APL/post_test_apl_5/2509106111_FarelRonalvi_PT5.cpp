...
void adminUbahServis(Servis daftar[], int jml) {
    ...
    if (!inp.empty()) {
        ...
        if (!validAngka) { tampilkanPesan("!", "Input biaya tidak valid! Harus berupa angka."); return; }
...
void userRequestServis(Servis daftar[], int &jml, User aktif) {
    ...
    tampilkanPesan("OK", "Yeyyyy Servis berhasil diajukan! (ID: " + to_string(daftar[idx].idServis) + ")");
...
void menuAwal() {
    ...
    do {
        ...
        switch (p) {
            ...
            case 0: tampilkanPesan("Terima kasih yaaaa abang/mba kuhhhh sudah mencobai program ini!"); break;
            default: tampilkanPesan("!", "Pilihan tidak valid!");
        }
    } while (p != 0);
}